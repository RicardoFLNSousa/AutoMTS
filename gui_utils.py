...
Button = Enum('Button', 'radio input checkbox multidrop daterange date time unidrop graph figure html link text upload dialog')
...
calendar = {'all':range(0,7),'weekday':range(0,5),'weekend':range(5,7)}
...
def button(button_id, title, values, radio, sel_options=None):
    ...
    #elif radio is Button.dialog:
        #button = dcc.ConfirmDialog(id=button_id, message='')
...
def get_states(parameters, skipdates=False, prefix=""):
    result = []
    if not skipdates: result = [State(parameters[0][0],'start_date'),State(parameters[0][0],'end_date')]
    start = 0 if skipdates else 1
    for i in range(start,len(parameters)): 
        result.append(State(parameters[i][0],'value'))
    return result
