...
def get_series_plot(series,title, missingSeries, figType):
    
    '''A: chart lines'''
    fig = tls.make_subplots(rows=1, cols=1, shared_xaxes=True, vertical_spacing=0.00000001, horizontal_spacing=0.001)
    for col in series.columns:
        fig.append_trace({'x':series.index, 'y':series[col], 'mode':'lines+markers', 'type':'scatter', 'name':col+ (' imputed' if(figType=='missings') else ' with outliers')}, 1, 1)
        fig.append_trace({'x':missingSeries.index, 'y':missingSeries[col], 'mode':'lines+markers', 'type':'scatter', 'name':col+(' with NA' if(figType=='missings') else '  detected outliers')}, 1, 1)
